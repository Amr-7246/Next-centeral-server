...
const MobileNavbar = () => {
  ...
  return (
    <div className=''>
      {/* mobile nav bar */}
        <nav ref={navRef}  className="bg-gray-900 shadow-lg fixed shadow-black/30 top-0 z-10 border-b border-black/50 flex-center md:!hidden !justify-between items-center h-fit w-[100%] py-5 px-5 ">
          ...
          <div className='flex flex-row gap-3'>
            ...
            <div onClick={() => {setIsOpend(true); navRef.current?.scrollIntoView({ behavior: "smooth" });}}  className="flex-center  group items-end flex-col w-[60px] gap-2 cursor-pointer">
                <span className={` ${IsOpend ? 'w-full  group-hover:w-1/2 ' : ' w-1/2 group-hover:w-full' } block  h-[1px] bg-[#B8860B] transition-all duration-800 ease-in-out `}></span>
                <span className="block w-full h-[1px] bg-[#B8860B] transition-all duration-300 ease-in-out"></span>
            </div>
          </div>
